...
// a = 0 moves cursor to the 1st line
// a = 1 moves cursor to the 2nd line
void MoveCursor(int a){
    if(a == 1){
        LCD_RS_PORT->OUT &= ~LCD_RS_MASK;
        LCD_DB_PORT->OUT = 0;
        LCD_DB_PORT->OUT |= SET_CURSOR_MASK;
        LCD_DB_PORT->OUT |= LINE2_OFFSET;

        LCD_EN_PORT->OUT |= LCD_EN_MASK;
        delayMicroSec(1);
        LCD_EN_PORT->OUT &= ~LCD_EN_MASK;
        instructionDelay(1, 0x00);
...
void writeInstruction(uint8_t mode, uint8_t instruction) {
    ...
    if (mode == DATA_MODE) {
        LCD_RS_PORT->OUT = (LCD_RS_PORT->OUT & 0xFF) | LCD_RS_MASK;
    } else {
        LCD_RS_PORT->OUT = (LCD_RS_PORT->OUT & 0x7F);
    }
    // pulse E to execute instruction on LCD
    // TODO set Enable signal high
    //      use bit-masking to avoid affecting other pins of port
        LCD_EN_PORT->OUT = (LCD_EN_PORT->OUT & 0xFF) | LCD_EN_MASK; //bit7
    delayMicroSec(1);
    // TODO set Enable signal low
    //      use bit-masking to avoid affecting other pins of port
    LCD_EN_PORT->OUT = (LCD_EN_PORT->OUT & 0xBF); //bit 6
